...
def RunExperiment(X_train, X_val, X_test, labels_train, labels_val, labels_test, train_loader, val_loader, epochs, torchOptim, lossFn, net, results_directory, name, device = 'cpu', saveAccToo = False):
...
def RunExperimentSGDT(X_train, X_val, X_test, labels_train, labels_val, labels_test, train_loader, val_loader, epochs, torchOptim, lossFn, net, results_directory, name, paramIndToThres, nzEntries, device = 'cpu', saveAccToo = False):
...
if __name__ == '__main__':
    ...
    from modelsSyntheticExp import SynSigmoid as netToUse
    ...
    mapLabelsSigmoid = pickle.load(open(read_path+'mapLabelsSigmoid.p', "rb"))
    mapInitWsPySigmoid = pickle.load(open(read_path+'mapInitWsPySigmoid.p', "rb"))
    mapInitWsGraphSigmoid = pickle.load(open(read_path+'mapInitWsGraphSigmoid.p', "rb"))
    mapInitWsGraphDSigmoid = pickle.load(open(read_path+'mapInitWsGraphDSigmoid.p', "rb"))

    suffix = 'ResultsSigmoid/'
    ...
    keyOnLabel = list(mapLabelsSigmoid.keys())[int(args.id)]
    print('We will run all experiments related to '+keyOnLabel)

    labels_train = mapLabelsSigmoid[keyOnLabel][0:100000]
    labels_val = mapLabelsSigmoid[keyOnLabel][100000:120000]
    labels_test = mapLabelsSigmoid[keyOnLabel][120000:220000]

    auxForKey = keyOnLabel.split('_')
    keyOnWeights = auxForKey[0]+'_'+auxForKey[1]

    weightsPytorch = mapInitWsPySigmoid[keyOnWeights]
    weightsGraphD = mapInitWsGraphDSigmoid[keyOnWeights]

    # tupleData = [X_train, X_val, X_test, labels_train, labels_val, labels_test, input_size, \
    #     first_layer, second_layer, third_layer, use_cuda, weightPytorch, weightGraphD]
    ...
    for lNab in lNabla:
        ...
        RunExperiment(X_train, X_val, X_test, labels_train, labels_val, labels_test, train_loader, val_loader, epochs, torchOptim, lossFn, net, results_directory, name, device = device)
        ## Run SGDT
        ...
        optimPerLayer = [methods[0], methods[0], methods[0]]
        torchOptim =  MixOptimizer(net.parameters(), optimPerLayer, partParams)
        RunExperimentSGDT(X_train, X_val, X_test, labels_train, labels_val, labels_test, train_loader, val_loader,epochs, torchOptim, lossFn, net, results_directory, name, paramIndToThres, nzVector, device = device)
